    ...
    <main className="min-h-screen bg-[#f7f8fc] text-slate-950">
      <header className="sticky top-0 z-50 border-b border-slate-200/70 bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex h-[72px] max-w-7xl items-center justify-between px-5 sm:px-8">
          ...
          <nav className="hidden items-center gap-8 text-sm font-medium text-slate-500 md:flex"><a href="#features" className="hover:text-indigo-600">Features</a><a href="#workflow" className="hover:text-indigo-600">How it works</a><a href="#pricing" className="hover:text-indigo-600">Pricing</a><Link href="/about" className="hover:text-indigo-600">About</Link></nav>
      ...
      <section id="pricing" className="border-y border-slate-200/70 bg-white px-5 py-20 sm:px-8 lg:py-24"><div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center"><SectionHeading icon={<IndianRupee className="size-4" />} eyebrow="Subscription pricing" title="A clear INR quotation for your school." text="SchoolDB subscriptions are tailored to the size and operational needs of each school." /><div className="rounded-2xl border border-slate-200/80 bg-slate-50/70 p-6 shadow-[0_10px_35px_rgba(15,23,42,0.05)] sm:p-8"><h3 className="text-lg font-bold text-slate-900">Custom annual subscription</h3><p className="mt-3 text-sm leading-6 text-slate-600">Pricing is quoted in Indian rupees (INR) based on student count, selected modules, onboarding requirements and subscription term. Your written quotation shows the exact amount, included services, taxes and payment schedule before you pay.</p><a href="mailto:harikiran.dev.schooldb@gmail.com?subject=SchoolDB%20subscription%20quotation" className="mt-6 inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-5 py-3 text-sm font-semibold text-white hover:bg-indigo-700">Request a quotation <ArrowRight className="size-4" /></a></div></div></section>

      <section id="demo" className="px-5 py-20 sm:px-8 lg:py-24"><div className="mx-auto max-w-7xl overflow-hidden rounded-[1.75rem] bg-slate-950 px-7 py-12 shadow-[0_20px_60px_rgba(15,23,42,0.14)] sm:px-10 lg:px-14 lg:py-14"><div className="mx-auto max-w-3xl text-center"><div className="mx-auto flex size-11 items-center justify-center rounded-xl bg-white/10 text-indigo-200 ring-1 ring-white/10"><Users className="size-5" /></div><div className="mt-5 text-[10px] font-bold uppercase tracking-[0.18em] text-indigo-300">See the product</div><h2 className="mt-3 text-3xl font-bold tracking-tight text-white sm:text-4xl">Show your school a simpler way to work.</h2><p className="mx-auto mt-4 max-w-2xl text-sm leading-6 text-slate-400 sm:text-base">Book a personalised SchoolDB demonstration and walk through the workflows your school uses every day.</p><div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row"><a href="mailto:harikiran.dev.schooldb@gmail.com?subject=SchoolDB%20demo%20request" className="inline-flex items-center justify-center gap-2 rounded-xl bg-white px-6 py-3.5 text-sm font-bold text-slate-950 hover:bg-indigo-50">Request a demo <ArrowRight className="size-4" /></a><Link href="/login" className="inline-flex items-center justify-center rounded-xl border border-white/15 bg-white/5 px-6 py-3.5 text-sm font-semibold text-white hover:bg-white/10">School login</Link></div></div></div></section>

      <footer className="border-t border-slate-200 bg-white px-5 py-9 sm:px-8"><div className="mx-auto grid max-w-7xl gap-6 text-xs text-slate-500 md:grid-cols-[1.2fr_1fr_auto] md:items-center"><div><div className="font-semibold text-slate-700">SchoolDB Education Technologies</div><div className="mt-1 text-slate-400">Udyam: UDYAM-AP-15-0043779</div></div><nav aria-label="Company and policy links" className="flex flex-wrap gap-x-5 gap-y-2"><Link href="/about" className="hover:text-indigo-600">About</Link><Link href="/contact" className="hover:text-indigo-600">Contact</Link><Link href="/privacy-policy" className="hover:text-indigo-600">Privacy</Link><Link href="/terms" className="hover:text-indigo-600">Terms</Link><Link href="/refund-policy" className="hover:text-indigo-600">Refunds</Link></nav><Link href="/login" className="font-semibold text-indigo-600 hover:text-indigo-700">School login <ArrowRight className="ml-1 inline size-3.5" /></Link></div></footer>
    </main>
  );
}
