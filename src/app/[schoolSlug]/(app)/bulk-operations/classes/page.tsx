...
function parseCsv(text: string) {
  ...
  lines.slice(1).forEach((line, index) => {
    ...
    if (
      ...
      displayOrder < 0 ||
      !Number.isInteger(sectionDisplayOrder) ||
      sectionDisplayOrder < 0
    ) {
      errors.push({
        row: index + 2,
        message: "Display order values must be non-negative whole numbers.",
      });
      return;
    }
    rows.push({
      ...
      sectionDisplayOrder: String(sectionDisplayOrder),
    });
  });
...
export default function BulkClassesPage() {
  ...
  async function handleFile(file: File) {
    ...
    if (!file.name.toLowerCase().endsWith(".csv")) {
      setFileError(
        "Upload a CSV file using the SchoolDB classes and sections template.",
      );
      return;
    }
    try {
  ...
  async function importClasses() {
    ...
    try {
      const data = await postImportInBatches<
        unknown,
        { classesCreated: number; sectionsCreated: number; skipped: number; errors: RowError[] }
      >({
        endpoint: "/api/v1/classes/bulk",
        bodyKey: "rows",
        rows: rows.map((row) => ({
            ...row,
            displayOrder: Number(row.displayOrder),
            sectionDisplayOrder: Number(row.sectionDisplayOrder),
          })),
        failureMessage: "Bulk class import failed.",
      });
      setResult(data);
      if (data.errors.length) setErrors(data.errors);
    } catch (error) {
      setFileError(
        error instanceof Error ? error.message : "Bulk class import failed.",
      );
    } finally {
      setImporting(false);
    }
      ...
      <Card className="premium-card overflow-hidden rounded-2xl border-0">
        <CardHeader className="border-b border-border/60 px-6 py-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle>Class & section import</CardTitle>
              <p className="mt-1 text-xs text-muted-foreground">
                CSV columns: name, code, sectionName, displayOrder,
                sectionDisplayOrder
              </p>
            </div>
          </div>
          ...
          {!hasFile && !fileError && (
            ...
            >
              ...
              <p className="mt-4 text-base font-bold">
                Upload classes & sections CSV
              </p>
              <p className="mt-1 max-w-md text-sm text-muted-foreground">
                Existing classes and sections are skipped safely; new records
            ...
            <>
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {rows.slice(0, 100).map((row, index) => (
                          <tr
                            key={`${row.name}-${row.sectionName}-${index}`}
              ...
              {result && (
                <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-4">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    {result.classesCreated} classes created ·{" "}
                    {result.sectionsCreated} sections created · {result.skipped}{" "}
                    existing/duplicate rows skipped
                  </p>
                </div>
              )}
