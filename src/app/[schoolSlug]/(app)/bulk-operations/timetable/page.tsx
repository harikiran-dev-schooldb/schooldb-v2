...
const TEMPLATE = [
  HEADERS.join(","),
  "2026-27,T001,Mathematics,Class 1,A,Period 1,MONDAY,true",
  "2026-27,T001,English,Class 1,A,Period 2,MONDAY,true",
].join("\r\n");
...
function parseCsv(text: string) {
  ...
  if (parseLine(lines[0]).map(clean).join("|") !== HEADERS.join("|"))
  ...
  lines.slice(1).forEach((line, index) => {
    ...
    if (
      ...
      errors.push({
        row,
        message:
          "Academic year, employee ID, subject, class, section, period and day are required.",
      });
      return;
    }
    if (
      ![
        ...
        "FRIDAY",
        "SATURDAY",
      ].includes(item.day.toUpperCase())
    ) {
      errors.push({ row, message: "Day must be MONDAY through SATURDAY." });
      return;
    }
    if (
      item.active &&
      !["true", "false", "1", "0", "yes", "no"].includes(
        item.active.toLowerCase(),
      )
    ) {
      errors.push({ row, message: "Active must be true or false." });
      return;
    }
    rows.push({
      ...item,
...
function downloadTemplate() {
  ...
  a.href = url;
  a.download = "schooldb-timetable-template.csv";
...
export default function BulkTimetablePage() {
  ...
  async function handleFile(file: File) {
    ...
    if (!file.name.toLowerCase().endsWith(".csv")) {
      setFileError("Upload a CSV file using the SchoolDB timetable template.");
      return;
    }
    try {
  ...
  return (
    <div className="space-y-8 pb-12">
      <PageHeader
        ...
        description="Import class and teacher timetable assignments using existing teacher allocations and periods."
        action={
      ...
      <p className="text-xs text-muted-foreground">
        Teacher allocations and periods must already exist. The import checks
        duplicate entries, teacher conflicts and class/section conflicts before
        writing.
      </p>
      <Card className="premium-card overflow-hidden rounded-2xl border-0">
        <CardHeader className="border-b border-border/60 px-6 py-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle>Timetable import</CardTitle>
              <p className="mt-1 text-xs text-muted-foreground">
                CSV: academicYear, employeeId, subject, className, section,
                period, day, active
              </p>
            </div>
          </div>
        ...
        <CardContent className="space-y-6 p-6">
          ...
          {!fileName && !fileError && (
            ...
            >
              ...
              <p className="mt-4 text-base font-bold">Upload timetable CSV</p>
            ...
            <>
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border/60 bg-muted/20 p-4">
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    {totalRows} rows · {rows.length} valid
                  </p>
                </div>
                  ...
                  <div className="max-h-[420px] overflow-auto">
                    <table className="w-full text-sm">
                      <thead className="sticky top-0 z-10 border-b border-border/60 bg-card">
                        <tr>
                          <th className="px-4 py-3 text-left text-[10px] uppercase tracking-wider text-muted-foreground">
                            #
                          </th>
                          {HEADERS.map((h) => (
                            <th
                              key={h}
                              className="px-4 py-3 text-left text-[10px] uppercase tracking-wider text-muted-foreground"
                            >
                              {h}
                            </th>
                          ))}
                      ...
                      <tbody>
                        {rows.slice(0, 100).map((r, i) => (
                          <tr
                            key={`${r.employeeId}-${r.period}-${r.day}-${i}`}
              ...
              {result && (
                <div className="rounded-2xl border border-border/60 bg-muted/20 p-4">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    {result.created} timetable entries created · {result.failed}{" "}
                    failed.
                  </p>
