...
const HEADERS = ["name", "code", "type", "displayOrder", "active"] as const;
const TEMPLATE = [
  HEADERS.join(","),
  "English,ENG,SCHOLASTIC,1,true",
  "Mathematics,MAT,SCHOLASTIC,2,true",
  "Physical Education,PE,CO_SCHOLASTIC,3,true",
].join("\n");
...
function parseCsv(text: string) {
  ...
  lines.slice(1).forEach((line, index) => {
    ...
    if (!row.name || row.name.length > 100) {
      errors.push({
        row: index + 2,
        message: "Subject name is required and must be 100 characters or less.",
      });
      return;
    }
    if (row.code.length > 20) {
      errors.push({
        row: index + 2,
        message: "Subject code must be 20 characters or less.",
      });
      return;
    }
...
function downloadTemplate() {
  ...
  anchor.href = url;
  anchor.download = "schooldb-subjects-template.csv";
  anchor.click();
...
export default function BulkSubjectsPage() {
  ...
  const duplicateCount = useMemo(() => {
    ...
    rows.forEach((row) =>
      counts.set(
        ...
        (counts.get(row.name.toLowerCase()) ?? 0) + 1,
      ),
    );
  ...
  async function handleFile(file: File) {
    ...
    if (!file.name.toLowerCase().endsWith(".csv")) {
      setMessage(
        "Please upload a CSV file using the SchoolDB subject template.",
      );
      return;
    }
    try {
  ...
  return (
    <div className="space-y-8 pb-12">
      <PageHeader
        ...
        title="Bulk Subjects"
        description="Upload subjects, validate them before import, and review the result."
        action={
      ...
      <Card className="premium-card overflow-hidden rounded-2xl border-0">
        <CardHeader className="border-b border-border/60 px-6 py-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle>Subject import</CardTitle>
              <p className="mt-1 text-xs text-muted-foreground">
                CSV columns: name, code, type, displayOrder, active
              </p>
            </div>
          </div>
        </CardHeader>
        <CardContent className="space-y-6 p-6">
          ...
          {!hasFile && (
            ...
            >
              ...
              <p className="mt-4 text-base font-bold">Upload subject CSV</p>
            ...
            <>
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border/60 bg-muted/20 p-4">
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    {totalRows} rows detected · {rows.length} valid
                  </p>
                </div>
                <div className="flex gap-2">
                  ...
                  {duplicateCount > 0 && (
                    <Badge variant="destructive">
                      {duplicateCount} duplicate names
                    </Badge>
                  )}
                  {errors.length > 0 && (
