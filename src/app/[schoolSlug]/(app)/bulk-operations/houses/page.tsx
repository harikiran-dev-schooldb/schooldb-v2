...
export default function BulkHousesPage(){
 ...
 return <div className="space-y-6"><PageHeader eyebrow="Bulk Operations" title="Student Houses" description="Create new houses or update existing houses by matching house name or code."/><Card><CardContent className="space-y-4 p-6"><div><p className="font-semibold">CSV data</p><p className="text-sm text-muted-foreground">Columns: name, code, color, description, displayOrder, active. Existing name/code is updated; otherwise a new house is created.</p></div><Textarea className="min-h-72 font-mono text-xs" value={csv} onChange={e=>setCsv(e.target.value)}/><div className="flex justify-end"><Button onClick={importRows} disabled={saving}>{saving?"Importing...":"Create / Update Houses"}</Button></div></CardContent></Card></div>;
}
