...
import { Cake, CalendarDays, Gift, Loader2, MessageCircle, Send } from "lucide-react";
import { toast } from "sonner";
import { PageHeader } from "@/components/common/PageHeader";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";

type BirthdayStudent={id:string;admissionNo:string;fullName:string|null;dob:string;imageUrl:string|null;whatsappOptIn:boolean;birthdayMonth:number;birthdayDay:number;nextBirthday:string;ageTurning:number;wishStatus:{status:string;sentCount:number;deliveredCount:number;readCount:number;failedCount:number}|null;enrollments:Array<{class:{name:string};section:{name:string}}>}
...
function StudentCard({student,showDate=false,onWish,sending=false}:{student:BirthdayStudent;showDate?:boolean;onWish?:(student:BirthdayStudent)=>void;sending?:boolean}){
 ...
 return <Link href={`students/${student.id}`}><Card className="h-full transition-all hover:-translate-y-0.5 hover:shadow-md"><CardContent className="flex items-center gap-4 p-5">
  ...
  <div className="mt-2 flex flex-wrap items-center gap-2">{student.whatsappOptIn?<Badge variant="secondary"><MessageCircle className="mr-1 size-3"/>WhatsApp enabled</Badge>:<Badge variant="outline">WhatsApp not opted in</Badge>}{student.wishStatus&&<Badge variant="outline">{student.wishStatus.status}</Badge>}</div></div>{onWish&&<button type="button" disabled={!student.whatsappOptIn||sending} onClick={e=>{e.preventDefault();e.stopPropagation();onWish(student);}} className="inline-flex h-9 items-center gap-1.5 rounded-lg border px-3 text-xs font-semibold disabled:opacity-50">{sending?<Loader2 className="size-3.5 animate-spin"/>:<Send className="size-3.5"/>}{student.wishStatus?"Retry":"Send Wish"}</button>}<Cake className="size-5 text-violet-500"/>
 </CardContent></Card></Link>;
}

export default function BirthdaysPage(){
 const[data,setData]=useState<ResponseData>(EMPTY);const[loading,setLoading]=useState(true);const[sendingId,setSendingId]=useState<string|null>(null);
 async function load(){try{const r=await fetch("/api/v1/birthdays",{cache:"no-store"});const j=await r.json();if(!r.ok||!j.success)throw new Error(j.message||"Failed to load birthdays.");setData(j.data);}catch(e){toast.error(e instanceof Error?e.message:"Failed to load birthdays.");}finally{setLoading(false);}}\n useEffect(()=>{void load()},[]);\n async function sendWish(student:BirthdayStudent){setSendingId(student.id);try{const r=await fetch(`/api/v1/birthdays/${student.id}/wish`,{method:"POST"});const j=await r.json();if(!r.ok||!j.success)throw new Error(j.message||"Unable to send birthday wish.");toast.success(j.message||"Birthday wish processed.");await load();}catch(e){toast.error(e instanceof Error?e.message:"Unable to send birthday wish.");}finally{setSendingId(null);}}
 return <div className="space-y-7 pb-10">
  ...
  {loading?<div className="flex min-h-52 items-center justify-center text-sm text-muted-foreground"><Loader2 className="mr-2 size-4 animate-spin"/>Loading birthdays...</div>:<>
   <section><div className="mb-3"><h2 className="text-lg font-bold">Today's Birthdays</h2><p className="text-sm text-muted-foreground">Students celebrating today.</p></div>{data.birthdays.length?<div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{data.birthdays.map(s=><StudentCard key={s.id} student={s} onWish={sendWish} sending={sendingId===s.id}/>)}</div>:<Card><CardContent className="flex min-h-36 flex-col items-center justify-center"><Gift className="size-8 text-violet-400"/><p className="mt-2 text-sm text-muted-foreground">No birthdays today.</p></CardContent></Card>}</section>
