      ...
      {!assignment ? (
        ...
        <div className="space-y-5">
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            <Card><CardContent className="p-6"><div className="flex items-start gap-4"><div className="flex size-12 shrink-0 items-center justify-center rounded-2xl bg-indigo-500/10 text-indigo-600 dark:text-indigo-400"><Clock3 className="size-5" /></div><div><p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Daily timing</p><p className="mt-2 text-lg font-bold">Pickup {assignment.stop.pickupTime || "To be confirmed"}</p><p className="mt-1 text-sm text-muted-foreground">Drop {assignment.stop.dropTime || "To be confirmed"}</p>{assignment.stop.monthlyFee && <p className="mt-3 text-sm font-semibold text-indigo-600 dark:text-indigo-400">₹{Number(assignment.stop.monthlyFee).toLocaleString("en-IN")} per month</p>}</div></div></CardContent></Card>
            <Card><CardContent className="p-6"><div className="flex items-start gap-4"><div className="flex size-12 shrink-0 items-center justify-center rounded-2xl bg-cyan-500/10 text-cyan-700 dark:text-cyan-400"><BusFront className="size-5" /></div><div><p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Assigned vehicle</p><p className="mt-2 text-lg font-bold">{assignment.route.vehicle?.name || assignment.route.vehicle?.registrationNo || "Vehicle to be assigned"}</p>{assignment.route.vehicle && <p className="mt-1 text-sm text-muted-foreground">{assignment.route.vehicle.registrationNo} · {assignment.route.vehicle.type.replaceAll("_", " ")}</p>}</div></div></CardContent></Card>
          </div>
...
function Contact({ icon: Icon, title, name, phone }: { icon: typeof UserRound; title: string; name: string; phone: string | null }) {
  return <div className="flex gap-4"><div className="flex size-11 shrink-0 items-center justify-center rounded-2xl bg-muted text-muted-foreground"><Icon className="size-5" /></div><div><p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">{title}</p><p className="mt-1 font-bold">{name}</p>{phone && <a href={`tel:+91${phone}`} className="mt-1 flex items-center gap-1.5 text-sm font-semibold text-indigo-600 dark:text-indigo-400"><Phone className="size-3.5" />+91 {phone}</a>}</div></div>;
}
