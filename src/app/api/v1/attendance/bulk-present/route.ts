...
export async function POST(req: Request) {
  return apiHandler(async () => {
    ...
    if (body.scope === "SECTION" && (!body.classId || !body.sectionId)) {
      return ApiResponse.error("Class and section are required.", 400);
    }

    const date = new Date(`${body.attendanceDate}T00:00:00.000Z`);
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (const group of groups.values()) {
        ...
        const studentIds = enrollments
          .filter(
            (enrollment) =>
              enrollment.classId === group.classId &&
              enrollment.sectionId === group.sectionId,
          )
