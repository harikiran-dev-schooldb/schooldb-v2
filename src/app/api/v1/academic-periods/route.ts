...
export async function POST(req: Request) {
  return apiHandler(async () => {
    ...
    const input = createAcademicPeriodSchema.parse(await req.json());

    const academicYear = await prisma.academicYear.findFirst({
      where: { id: input.academicYearId, schoolId: tenant.schoolId },
    });
    ...
    const existing = await prisma.academicPeriod.findFirst({
      where: {
        ...
        OR: [{ name: input.name }, { sequence: input.sequence }],
      },
    });
