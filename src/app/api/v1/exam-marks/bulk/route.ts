  ...
  return apiHandler(async () => {
    ...
    for (let i = 0; i < prepared.length; i += 1) {
      ...
      const enrollment = enrollmentByKey.get(
        [student.id, year.id, classRecord.id, sectionId ?? ""].join(":"),
      );
      if (!enrollment)
        throw new Error(
          `Row ${rowNumber}: Student ${row.admissionNo} is not enrolled in the selected class/section for ${row.academicYear}.`,
        );
      const schedule = scheduleByKey.get(
        [exam.id, classRecord.id, sectionId ?? "", subject.id].join(":"),
      );
      if (!schedule)
        throw new Error(
          `Row ${rowNumber}: Exam schedule not found for ${row.subjectName}.`,
        );
