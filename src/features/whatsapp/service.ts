...
export async function createWhatsappCampaign(input: CreateCampaignInput) {
  ...
  if (recipients.size === 0) throw new Error("No valid student or guardian mobile numbers were found for this audience.");
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (input.automationKey && error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
...
export async function queueAutomatedWhatsappAlert(input: AutomatedAlertInput) {
  ...
  if (!templateName) {
    console.warn("[whatsapp-automation] Template is not configured", { sourceType: input.sourceType, sourceId: input.sourceId });
    return null;
  }
  try {
...
async function sendTemplate(phone: string, templateName: string, title: string, message: string) {
  ...
  try {
    ...
    if (!response.ok) {
      const providerError = [data.error?.message, data.error?.error_data?.details]
        .filter(Boolean)
        .join(" — ");
...
export async function processWhatsappCampaignBatch(schoolId: string, campaignId: string, batchSize = 20) {
  const campaign = await prisma.whatsappCampaign.findFirst({
    where: { id: campaignId, schoolId, status: { notIn: ["CANCELLED", "COMPLETED"] }, scheduledAt: { lte: new Date() } },
  ...
  const recipients = await prisma.whatsappRecipient.findMany({
    where: { campaignId, schoolId, status: { in: ["QUEUED", "FAILED"] }, attempts: { lt: 3 } },
  ...
  await prisma.$transaction([
    ...
    prisma.whatsappRecipient.updateMany({ where: { id: { in: recipients.map((item) => item.id) } }, data: { status: "SENDING", lastAttemptAt: new Date(), attempts: { increment: 1 } } }),
  ]);

  const groups = Array.from({ length: Math.ceil(recipients.length / 5) }, (_, index) => recipients.slice(index * 5, index * 5 + 5));
  for (const group of groups) {
    await Promise.all(group.map(async (recipient) => {
      try {
        const providerMessageId = await sendTemplate(recipient.phone, campaign.templateName, campaign.title, campaign.message);
        ...
        await prisma.whatsappRecipient.update({ where: { id: recipient.id }, data: { status: "FAILED", errorMessage: (error instanceof Error ? error.message : "Delivery failed").slice(0, 500) } });
      }
    }));
  }
  ...
  const [sentCount, deliveredCount, readCount, failedCount, retryableCount] = await Promise.all([
    ...
    prisma.whatsappRecipient.count({ where: { campaignId, status: { in: ["QUEUED", "FAILED"] }, attempts: { lt: 3 } } }),
  ]);
  const status = retryableCount > 0 ? "QUEUED" : failedCount === 0 ? "COMPLETED" : sentCount === 0 ? "FAILED" : "PARTIAL";
  await prisma.whatsappCampaign.update({
    where: { id: campaignId },
    data: { sentCount, deliveredCount, readCount, failedCount, status, completedAt: retryableCount === 0 ? new Date() : null },
  });
