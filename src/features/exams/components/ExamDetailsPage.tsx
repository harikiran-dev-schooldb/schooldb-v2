...
export function ExamDetailsPage({ schoolSlug, examId }: Props) {
  ...
  const totalSections = useMemo(
    () => new Set(schedules.map((s) => s.section?.id).filter(Boolean)).size,
    [schedules],
  );
        ...
        <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              variant="outline"
              onClick={() =>
                router.push(`/${schoolSlug}/exams/${examId}/timetable`)
              }
            >
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[
          [
            "Schedules",
            schedules.length,
            <ClipboardList key="i" className="size-5" />,
          ],
          [
            "Subjects",
            totalSubjects,
            <ClipboardPenLine key="i" className="size-5" />,
          ],
          ["Classes", totalClasses, <Users key="i" className="size-5" />],
          [
            "Sections",
            totalSections,
            <CheckCircle2 key="i" className="size-5" />,
          ],
        ...
        <CardContent className="p-6">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-3">
            {[
              ...
              <div
                key={label}
                className="rounded-xl border border-border/60 bg-muted/[0.3] p-4"
              >
                  ...
                  {schedules.map((schedule) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-4">
                        <div>{formatDate(schedule.examDate)}</div>
                        <div className="text-xs text-muted-foreground">
